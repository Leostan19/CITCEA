...
def save_operation(allResults, AllInputs, save_folder):
    ...
    if AllInputs.EV.hay == 1 and AllInputs.EV.immediate0_smart1 == 1: # Flexibility: EV smart charging
        table_EV_P = dataframe_from_dict_loads(allResults.EV.P, l_Mev, l_t)
        table_EV_SOC = dataframe_from_dict_loads(allResults.EV.SOC, l_Mev, l_t)

    dict_cL = {}
    dict_EV = {}
    for Mev in l_Mev:
        # 列名可以自己定，这里用 P_x / SOC_x
        dict_EV[f'P_{Mev}'] = table_EV_P[Mev]
        dict_EV[f'SOC_{Mev}'] = table_EV_SOC[Mev]

    table_EV = pandas.DataFrame(dict_EV, index=l_t)
